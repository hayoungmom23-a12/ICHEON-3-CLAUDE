...
> 2026-09-18 개발 계획(plan) R2 준비도 점검에서 문서 충돌 3건을 고쳤다: 3절 코스 표에서 표 밖으로 밀려 있던 서이천 코스 줄을 표 안으로 올렸고(표가 코스 4개로 보이던 문제), 5절 권역 항목의 "소개 한 줄"을 뺐으며(6절 제외 확정과 맞춤), 8절의 "화면 3개"를 "화면 2개와 두 화면 공통 하단 안내"로 고쳤다(FRD·TRD의 세는 방식과 맞춤). 내용을 새로 넣거나 뺀 것은 없다.
>
> 2026-09-18 기능 명세(FRD) R1에서 확정 시안 3장과 대조해 갱신: 권역 소개 3줄과 하단 안내의 확인 기준 줄을 뺐고, 네이버지도 임베드를 제외 범위로 옮겼으며, 시안에서 정해진 화면 규칙을 3절에 추가하고 목표일을 다시 잡았다. 2026-09-15 간략화 재작성, 같은 날 화면 시안 확인 후 사진·기준일 표시·팁 이름을 보완. 2026-09-09 서비스 기획서가 고른 "다음 주에 실제로 만들 핵심 기능 1개"로 범위를 되돌리고, 직접 코스 만들기·그림지도 동선·공유 기능은 v2 후보로 옮겼다. 원본 PRD는 `../ICHEON/docs/prd.md`에 그대로 둔다.
...
### 제외 확정 네이버지도 임베드 (2026-09-18)

코스 목록 페이지에 이천 지도 한 장을 넣고 스팟 5곳의 위치를 표시하는 안이었다. 네이버지도 API(네이버 지도를 내 페이지에 불러오는 공식 통로)를 쓰려면 네이버클라우드플랫폼 가입, Client ID 발급, 사이트 주소 등록이 필요하다.

포함 조건이었던 "2026-09-16(수) 오전 30분 안에 키 발급"이 지났고 키를 받지 않았으므로, PRD가 스스로 정한 규칙대로 **넣지 않는다**. 시안 `design/01-코스목록.png`의 "스팟 5곳 위치" 지도 상자도 함께 뺀다. 위치 확인은 주차장별 "지도 열기" 링크가 맡으며, 이것으로 서비스는 성립한다. 키를 나중에 받으면 같은 자리에 다시 넣을 수 있다.

### 시안 대조로 확정한 화면 규칙 (2026-09-18 기능 명세 R1)

확정 시안 PNG 3장을 PRD와 한 줄씩 맞춰 본 결과 정해진 규칙이다. 자세한 기능 목록은 `docs/frd.md`에 있다.

- 코스 상세 맨 위에 "← 코스 목록" 글자 링크를 둔다. 브라우저 뒤로 가기도 그대로 쓴다.
- 스팟 대표 사진 위에 출처·저작자 글씨를 겹쳐 표시한다. 예: "설봉호 · Wikimedia Commons".
- 시설의 전화번호는 별도 줄이 아니라 운영시간 요약 줄 끝에 함께 적는다. 예: "10:00~18:00 · 월요일 휴관 · 문의 031-633-9734".
- 코스 카드 위 소제목과 스팟 카드의 탭에 개수를 적는다. "취향별 추천 코스 5개", "추천 장소 6", "방문 팁 3". "추천 장소 N"의 N은 그 스팟의 전체 시설 수다.
- 스팟 카드 오른쪽 위에 "접기 ∧" 글자 토글을 둔다.
- 상시 개방 시설은 지도 링크를 적은 것에만 "지도 열기" 버튼을 붙이고, 안 적은 시설은 버튼 없이 한 줄 소개만 둔다.
- 취향 필터를 켜면 코스 카드와 "코스 5개 한눈에" 요약 줄을 함께 거른다.
- 답사하지 않은 정보의 화면 배지는 짧게 "지도 기준"으로 쓰고, 뜻풀이는 하단 안내에만 둔다.
- 없는 코스 주소로 들어오면 코스 목록으로 보낸다.
- 스팟 카드가 2개인 코스에서는 두 카드를 동시에 펼쳐 둘 수 있다.
...
1. 코스 목록 화면: 한 페이지에서 코스 5개를 한눈에 본다. 위에서 아래로 히어로(서비스 이름 + 한 줄 설명), 코스 요약 목록 5줄, 취향 3택 버튼, 코스 카드 5장, 바깥 링크 타일 2개, 하단 안내 순서다. 카드에는 코스 이름, 취향 트랙, 권역, 스팟 이름과 추천 주차장, "비 오는 날 OK" 태그를 둔다. 코스 카드 위 소제목에는 지금 보이는 코스 개수를 적고, 취향 필터를 켜면 요약 목록 5줄과 코스 카드를 함께 걸러 소제목 숫자도 바꾼다. (2026-09-18 기능 명세 R1: 3개 권역 소개 한 줄씩은 코스 줄과 카드마다 권역 이름이 이미 붙어 있어 뺐고, 조건부 지도는 제외 범위로 옮겼다)
...
3. 정보 출처·기준일 안내: 페이지 하단 영역. 서비스 한 줄 소개, "운영 정보는 2026-09-04 기준이며 방문 전 각 시설의 공식 페이지에서 확인" 한 줄, "지도 기준" 표시의 뜻 한 줄, 모두 3줄이다. 기준일은 데이터 파일의 값 하나를 바꾸면 갱신된다. (2026-09-18 기능 명세 R1: 별도의 "공식 페이지·전화 확인 기준" 줄은 기준일 줄과 같은 말이라 뺐다)
...
- 권역: 이름 (권역 소개 한 줄은 2026-09-18 제외 확정. 6절 참고)
...
- 직접 그린 이천 개략 지도와 스팟 사이 동선 표시 (위치는 주차장별 네이버지도 링크로 대체). 스팟 안의 진입로를 그린 "방문자 시점 약도"는 2026-09-18부터 선택 항목으로 포함 범위다.
- 코스 목록의 네이버지도 임베드 (2026-09-18 제외 확정. 키 발급 조건이 지났다. 자세한 내용은 3절)
- 코스 목록의 권역 소개 3줄 (2026-09-18 제외 확정. 코스 줄과 카드마다 권역 이름이 이미 붙어 있다)
...
- 2026-09-23(수)까지 화면 구현과 데이터 채우기를 마치고 2026-09-24(목)에 배포한다. (2026-09-18 기능 명세 R1에서 다시 잡은 목표일. 작업 가능 시간인 화·목 14시 이후와 수요일 종일에 맞췄다. 09-18(금) 문서 3~5단계, 09-22(화) 오후 구현 시작, 09-23(수) 종일 구현과 데이터, 09-24(목) 오후 점검과 배포.)

## 8 예상 복잡도

작음 — 화면 2개(코스 목록·코스 상세)와 두 화면 공통 하단 안내, 정적 데이터 파일 3개이고 로그인·결제·실시간 연동·상태 관리가 없다. 복잡도를 높이는 요인은 코드가 아니라 데이터다. 2026-09-18 디자인 단계에서 3분할 탭, 요약 목록, 링크 타일, 약도 구역이 추가되어 화면 요소는 늘었지만 데이터는 약도 이미지(선택)와 링크 주소 2개만 는다. 식당 운영 정보를 저장하지 않으므로 확인 대상은 스팟 5곳(설봉공원, 이천농업테마공원, 이천목재문화체험장, 덕평공룡수목원, 이진상회)의 주차장 7곳과 시설 12곳 안팎뿐이지만, 주차장 구분과 운영시간·휴무를 공식 자료로 확인하는 일과 화·목 14시 이후만 가능한 작업 시간이 제약이다.

## 아직 확인할 결정

- 이천농업테마공원의 두 번째 주차장 이름과 남부 만들기 코스에서 가는 공원 시설의 이름 (구현 데이터 작성 때 답사 메모로 확정). 2026-09-18 결정: 데이터를 쓰는 시점까지 이름이 확인되지 않으면 그 줄을 비워 두지 않고 통째로 빼며, 남부 코스 두 개는 "안내센터 옆 주차장" 한 줄만 둔다. 지어낸 이름은 쓰지 않는다.

## 확정 시 유지하는 운영 가정

- 구현 완성은 2026-09-23(수), 배포는 2026-09-24(목)이다. 작업 가능 시간은 화·목 14시 이후, 수요일 종일이다. (2026-09-18 갱신)
...
- 지도는 주차장별 네이버지도 링크가 전부다. 네이버지도 임베드는 2026-09-18에 제외로 확정했다.
